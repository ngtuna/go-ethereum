	...
	if eth.chainConfig.Posv != nil {
			...
			if prevEpoc >= 0 {
				...
				if len(penSigners) > 0 {
					...
					for i := prevEpoc; i < blockNumberEpoc; i++ {
						...
						if len(penSigners) > 0 {
							signedMasternodes, err := contracts.GetSignersFromContract(c, state, block)
							if err != nil {
								return nil, err
							}
		...
		c.HookReward = func(state *state.StateDB, chain consensus.ChainReader, header *types.Header) (error, map[string]interface{}) {
			...
			if number > 0 && number-rCheckpoint > 0 && foundationWalletAddr != (common.Address{}) {
				...
				signers, err := contracts.GetRewardForCheckpoint(c, chain, number, rCheckpoint, totalSigner, state)
				...
				if len(signers) > 0 {
					for signer, calcReward := range rewardSigners {
						err, rewards := contracts.CalculateRewardForHolders(c, foundationWalletAddr, state, signer, calcReward)
						if err != nil {
